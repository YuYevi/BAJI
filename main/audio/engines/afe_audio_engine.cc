...
bool AfeAudioEngine::Initialize(AudioCodec* codec, int frame_duration_ms, srmodel_list_t* models_list) {
    ...
    const afe_type_t afe_type = codec_->input_reference() ? AFE_TYPE_VC : AFE_TYPE_SR;
    afe_config_t* afe_config = afe_config_init(
        input_format.c_str(), models_, afe_type, AFE_MODE_LOW_COST);
    ...
    ESP_LOGI(TAG, "Initialized AFE, detector: %s, feed: %d, fetch: %d",
...
WakeWord* AfeAudioEngine::GetWakeWord() {
    if (wake_detector_ == WakeDetector::kMultiNet) {
        return custom_wake_word_.get();
    }
    return nullptr;
}
