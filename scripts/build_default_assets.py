...
def normalize_extra_files_dirs(extra_files_dirs):
    """Normalize --extra_files arguments into a flat directory list."""
    ...
    normalized_dirs = []
    for item in extra_files_dirs:
        if not item:
            continue
        normalized_dirs.extend([part for part in item.split(';') if part])
...
def process_extra_files(extra_files_dirs, assets_dir):
    ...
    extra_files_dirs = normalize_extra_files_dirs(extra_files_dirs)
    if not extra_files_dirs:
        return []
    ...
    for extra_files_dir in extra_files_dirs:
        ...
        for root, dirs, files in os.walk(extra_files_dir):
            for file in files:
                ...
                # AI chat has moved to MJPEG, so these GIFs no longer need assets partition space.
                if file in SKIP_DEFAULT_EXTRA_FILES:
                    print(f"Skipping unused extra file: {file}")
                    continue
    ...
    if extra_files_list:
        print(f"Processed {len(extra_files_list)} extra files from: {', '.join(extra_files_dirs)}")
...
def get_text_font_path(builtin_text_font, BAJI_fonts_path):
    """
    ...
    font_path = os.path.join(BAJI_fonts_path, 'cbin', font_name)
...
def get_emoji_collection_path(default_emoji_collection, BAJI_fonts_path, project_root=None):
    """
    Get the emoji collection path if needed
    Returns the emoji directory path or None if no emoji collection is needed
    
    Supports:
    - PNG emoji collections from BAJI-fonts (e.g., emojis_32, twemoji_64)
    - GIF emoji collections from BAJI-fonts (e.g., noto-emoji_128, noto-emoji_64)
    ...
    emoji_path = os.path.join(BAJI_fonts_path, 'png', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path
    
    # Try GIF emoji collections (e.g., noto-emoji_128, noto-emoji_64, noto-emoji_32)
    emoji_path = os.path.join(BAJI_fonts_path, 'gif', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path
...
def main():
    ...
    parser.add_argument('--BAJI_fonts_path', help='Path to BAJI-fonts component directory')
    parser.add_argument('--extra_files', nargs='+', help='Path(s) to extra files directories to be included in assets')
    
    args = parser.parse_args()
    
    # Set default paths if not provided
    if not args.esp_sr_model_path or not args.BAJI_fonts_path:
        ...
        if not args.BAJI_fonts_path:
            managed_fonts = os.path.join(project_root, "managed_components", "78__xiaozhi-fonts")
            component_fonts = os.path.join(project_root, "components", "xiaozhi-fonts")
            args.BAJI_fonts_path = managed_fonts if os.path.isdir(managed_fonts) else component_fonts
    ...
    text_font_path = get_text_font_path(args.builtin_text_font, args.BAJI_fonts_path)
    
    # Get emoji collection path if needed
    # Calculate project root from script location for otto-gif support
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    emoji_collection_path = get_emoji_collection_path(args.emoji_collection, args.BAJI_fonts_path, project_root)
