...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // After validating and saving the credentials, leave config mode so
            // the normal station workflow can reconnect using the saved WiFi.
            if (this_->on_exit_requested_) {
