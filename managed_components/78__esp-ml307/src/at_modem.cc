...
NetworkStatus AtModem::WaitForNetworkReady(int timeout_ms) {
    ...
    const TickType_t timeout_ticks = has_timeout ? pdMS_TO_TICKS(timeout_ms) : portMAX_DELAY;
    auto remaining_ticks = [&]() -> TickType_t {
        if (!has_timeout) {
    ...
    auto remaining_timeout_ms = [&]() -> int {
        TickType_t ticks = remaining_ticks();
        ...
        int ms = static_cast<int>(ticks * portTICK_PERIOD_MS);
        return ms > 0 ? ms : 1;
    };
    ...
    for (int i = 0; !has_timeout || i < 10; i++) {
        ...
        TickType_t remain = remaining_ticks();
        if (remain == 0) {
            return NetworkStatus::ErrorTimeout;
        }
        if (remain != portMAX_DELAY && wait_ticks > remain) {
            wait_ticks = remain;
        }
    ...
    if (!at_uart_->SendCommand("AT+CEREG=2", command_timeout_ms > 0 ? command_timeout_ms : 1000)) {
        return command_timeout_ms == 0 ? NetworkStatus::ErrorTimeout : NetworkStatus::Error;
    }
    command_timeout_ms = remaining_timeout_ms();
    if (command_timeout_ms == 0) {
        return NetworkStatus::ErrorTimeout;
    }
    if (!at_uart_->SendCommand("AT+CEREG?", command_timeout_ms > 0 ? command_timeout_ms : 1000)) {
        return command_timeout_ms == 0 ? NetworkStatus::ErrorTimeout : NetworkStatus::Error;
    }
    
    TickType_t timeout = portMAX_DELAY;
    if (has_timeout) {
        timeout = remaining_ticks();
        if (timeout == 0) {
            return NetworkStatus::ErrorTimeout;
        }
    }
...
bool AtModem::IsBusyForSignalQuery() const {
    return false;
}

int AtModem::GetCsq() {
    // ML307 shares one AT channel across MQTT/HTTP/control queries. During active
    // HTTP transfers, polling CSQ can interfere with response delivery, so keep
    // the last known signal value until the channel becomes idle again.
    if (IsBusyForSignalQuery()) {
        return csq_;
    }
