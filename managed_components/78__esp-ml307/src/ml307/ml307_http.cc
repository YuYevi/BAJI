...
std::atomic<int> Ml307Http::active_transfer_count_{0};
            ...
            if (arguments[1].int_value == http_id_) {
                ...
                if (type == "header") {
                    ...
                    if (arguments.size() >= 6) {
                        ...
                        // Some firmware versions omit the trailing empty data field on EOF.
                    } else {
                        error_code_ = 7;
                        xEventGroupSetBits(event_group_handle_, ML307_HTTP_EVENT_ERROR);
                        ESP_LOGE(TAG, "Malformed content URC, argc=%u", arguments.size());
                        Close();
                        return;
                    }
...
bool Ml307Http::HasActiveTransfer() {
    return active_transfer_count_.load() > 0;
}
...
bool Ml307Http::Open(const std::string& method, const std::string& url) {
    ...
    if (!counted_active_transfer_) {
        active_transfer_count_.fetch_add(1);
        counted_active_transfer_ = true;
    }
...
void Ml307Http::Close() {
    ...
    if (counted_active_transfer_) {
        ...
        counted_active_transfer_ = false;
    }
    eof_ = true;
    cv_.notify_all();
