        ...
        if (command == "MIPOPEN" && arguments.size() == 2) {
            ...
            if (arguments[0].int_value == tcp_id_) {
                if (arguments[1].int_value >= 0) {
                    ...
                    xEventGroupSetBits(event_group_handle_, ML307_TCP_SEND_FAILED);
                }
            }
...
int Ml307Tcp::Send(const std::string& data) {
    ...
    while (total_sent < data.size()) {
        ...
        for (int attempt = 0; attempt < TCP_SEND_RETRY_COUNT; ++attempt) {
            xEventGroupClearBits(event_group_handle_, ML307_TCP_SEND_COMPLETE | ML307_TCP_SEND_FAILED);
            ...
            if (!at_uart_->SendCommand(command, TCP_SEND_COMMAND_TIMEOUT_MS, false)) {
                ESP_LOGW(TAG, "MIPSEND command timeout on attempt %d/%d", attempt + 1, TCP_SEND_RETRY_COUNT);
                if (!connected_) {
                    Disconnect();
                    return -1;
                }
            ...
            auto bits = xEventGroupWaitBits(event_group_handle_,
                ML307_TCP_SEND_COMPLETE | ML307_TCP_SEND_FAILED,
                pdTRUE, pdFALSE, pdMS_TO_TICKS(TCP_SEND_RESULT_TIMEOUT_MS));
            if (bits & ML307_TCP_SEND_COMPLETE) {
                sent = true;
                break;
            }
            if (bits & ML307_TCP_SEND_FAILED) {
                ESP_LOGW(TAG, "MIPSEND rejected by modem on attempt %d/%d, err=%d",
                    attempt + 1, TCP_SEND_RETRY_COUNT, last_error_);
            } else {
                ESP_LOGW(TAG, "No send confirmation received on attempt %d/%d",
                    attempt + 1, TCP_SEND_RETRY_COUNT);
            }
            if (!connected_) {
                Disconnect();
                return -1;
